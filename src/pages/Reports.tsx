...
export default function Reports() {
  ...
  const { data, isLoading } = useReportsData(dateRange.from!, dateRange.to!);
          ...
          <div className="flex flex-wrap gap-2">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem
                  onClick={() =>
                    downloadCSV(data?.rawData.orders || [], "orders_report")
                  }
                >
                ...
                <DropdownMenuItem
                  onClick={() =>
                    downloadCSV(data?.rawData.expenses || [], "expenses_report")
                  }
                >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 md:gap-4 print:grid-cols-5 print:gap-2">
          {[
            ...
            },
            {
              ...
              subtitle: `${(data?.kpis.grossMargin || 0).toFixed(1)}% margin`,
              className:
                (data?.kpis.grossProfit || 0) >= 0
                  ? "text-green-600"
                  : "text-destructive",
            ...
            },
            {
              icon: Package,
              label: "Units Sold",
              value: data?.kpis.totalUnitsSold || 0,
              tooltip: "Total product units sold across all orders",
            },
            {
              icon: Target,
              label: "Avg Order",
            ...
            },
            {
              ...
              tooltip: "Cost of Goods Sold - total cost price of items sold",
            },
            {
              icon: Receipt,
              label: "Expenses",
              ...
              tooltip:
                "Operating expenses (stall fees, travel, supplies, etc.)",
            },
            {
              icon: PiggyBank,
              label: "Net Profit",
              value: formatCurrency(data?.kpis.netProfit || 0),
              className:
                (data?.kpis.netProfit || 0) >= 0
                  ? "text-green-600"
                  : "text-destructive",
              tooltip: "Gross Profit minus Expenses = Final profit",
            },
            {
              icon: BarChart3,
              label: "Margin",
              value: `${(
                ((data?.kpis.netProfit || 0) / (data?.kpis.totalRevenue || 1)) *
                100
              ).toFixed(1)}%`,
              className:
                ((data?.kpis.netProfit || 0) / (data?.kpis.totalRevenue || 1)) *
            ...
            <Card
              key={i}
              className="print:border print:shadow-none group relative"
            ...
            >
              <CardContent className="pt-3 px-2 pb-2 md:pt-4 md:px-4 md:pb-3 print:p-2">
                ...
                <p
                  className={`text-sm md:text-xl font-semibold mt-0.5 md:mt-1 truncate print:text-base ${
                    (kpi as any).className || ""
                  }`}
                >
                  {kpi.value}
                </p>
          ...
          <Card className="lg:col-span-2 print:break-inside-avoid">
            <CardHeader className="pb-2 md:pb-4">
              ...
              <p className="text-xs md:text-sm text-muted-foreground">
                Revenue vs Costs with Profit trend
              </p>
            </CardHeader>
                ...
                <BarChart data={data?.topProducts || []} layout="vertical">
                  ...
                  <Tooltip
                    ...
                    labelFormatter={(label) => {
                      const product = data?.topProducts?.find(
                        (p) => p.name === label
                      );
                      return product
                        ? `${label} (${product.unitsSold} units sold)`
                        : label;
                    }}
                  />
